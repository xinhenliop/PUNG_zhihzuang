...
//讲一下我个人对第二个参数的看法
//数组[]应该是一种语法糖
//char **argv是指向一个char指针的指针
//当使用argv[1]的时候会自动进行运算获取下一个数据

/*
int main(int argc, char* argv[]){
	char* ip = getip("w.eydata.net");
	if(ip != NULL){
		printf("%s\n",ip);
		free(ip);
	}
	//该函数参数为，域名，后缀，POST参数，易游公告不需要参数可以瞎填
	//http:// w.eydata.net / 07c99eb17c67ce7f 按照格式自己分割字符串，后缀需要填写完整
	char* xyzw = httppost("w.eydata.net", "07c99eb17c67ce7f", "账号：282199034密码：15838305921NM$L");
	if(xyzw != NULL){
		printf("%s\n", xyzw);
		free(xyzw);
	}
...
char* httppost(char* hostname, char* url, char* cs){
	...
	if (FD_ISSET(sockfd, &fds)) { //测试sock是否可读，即是否网络上有数据
		...
		//printf("%s\n--\n",xyt);
		char* xylen = strstrstr(xyt,(char*)"Content-Length: ",(char*)"\n");
		char* xyzw;
		if(xylen == NULL){
			//如果读取正文长度失败则进入分段读取模式
			char hexlen[8];
